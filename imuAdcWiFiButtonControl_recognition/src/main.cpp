...
*/
static uint32_t num_pkt_flag=0; //utilizzato per far capire al server che è finito l'invio dei pacchetti di un gesto
static uint32_t num_pkt_inviati=1;
static uint32_t num_pkt_negato=1;
  ...
  switch (state_core0){
    ...
    case STATE_CORE0_PREPARING_STOP:
      ...
      if(imu_level_queue > 0){
        ...
        num_pkt_flag=0;
        /* Num_pkt */
        packet_buffer[offset++] = num_pkt_flag & 0xFF;
        packet_buffer[offset++] = (num_pkt_flag >> 8) & 0xFF;
        packet_buffer[offset++] = (num_pkt_flag >> 16) & 0xFF;
        packet_buffer[offset++] = (num_pkt_flag >> 24) & 0xFF; 
        offset+=((32+6*16)*N_SAMPLES_IMU_PACKET+(32+4*12)*N_SAMPLES_ADC_PACKET)/8;
        /* ~Num_pkt */
        num_pkt_negato = ~num_pkt_flag;
        packet_buffer[offset++] = num_pkt_negato & 0xFF;
        packet_buffer[offset++] = (num_pkt_negato >> 8) & 0xFF;
        packet_buffer[offset++] = (num_pkt_negato >> 16) & 0xFF;
        packet_buffer[offset++] = (num_pkt_negato >> 24) & 0xFF;
